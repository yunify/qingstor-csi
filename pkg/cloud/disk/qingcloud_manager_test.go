...
func TestQingCloudManager_CreateVolume(t *testing.T) {
	...
	for _, test := range tests {
		volId, err := cloud.CreateVolume(test.volName, test.volSize, test.volRepl, test.volType, test.volZone)
		if err != nil {
			if !test.isError {
...
func TestQingCloudManager_FindVolume(t *testing.T) {
	...
	for _, test := range tests {
		volInfo, err := cloud.FindVolume(test.volId)
		if err != nil {
			if !test.isError {
				t.Errorf("testcase %s: expect error %t, but actually error: %s", test.name, test.isError, err)
			}
			...
			if *cloud.VolumeName != test.volName {
...
func TestQingCloudManager_FindVolumeByName(t *testing.T) {
	...
	for _, test := range tests {
		volInfo, err := cloud.FindVolumeByName(test.volName)
		if err != nil {
			if !test.isError {
				t.Errorf("testcase %s: expect error %t, but actually error: %s", test.name, test.isError, err)
			}
...
func TestQingCloudManager_FindInstance(t *testing.T) {
	...
	for _, test := range tests {
		instInfo, err := cloud.FindInstance(test.instId)
		if err != nil {
			if !test.isError {
				t.Errorf("testcase %s: expect error %t, but actually error: %s", test.name, test.isError, err)
			}
...
func TestQingCloudManager_DeleteVolume(t *testing.T) {
	...
	for _, test := range tests {
		err := cloud.DeleteVolume(test.volid)
...
func TestQingCloudManager_CreateVolumeFromSnapshot(t *testing.T) {
	...
	for _, test := range tests {
		volId, err := cloud.CreateVolumeFromSnapshot(test.volName, test.snapId, test.zone)
		if err != nil {
			if !test.isError {
				t.Errorf("testcase %s: expect error %t, but actually error: %s", test.name, test.isError, err)
			}
...
func TestQingCloudManager_FindTag(t *testing.T) {
	...
	for _, v := range tests {
		tagInfo, err := cloud.FindTag(v.tagId)
...
func TestQingCloudManager_IsValidTags(t *testing.T) {
	...
	for _, test := range tests {
		res := cloud.IsValidTags(test.tagId)
		if test.isValid != res {
...
func TestQingCloudManager_AttachTags(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			resourceType: cloud.ResourceTypeVolume,
			isError:      false,
		},
		{
			name:         "re-attach tags",
		...
		},
		{
			...
			tagId:        []string{tagId},
			resourceId:   resourceId,
			resourceType: cloud.ResourceTypeSnapshot,
			isError:      true,
		},
		{
	...
	for _, v := range tests {
		err := cloud.AttachTags(v.tagId, v.resourceId, v.resourceType)
...
func TestQingCloudManager_CloneVolume(t *testing.T) {
	...
	for _, test := range tests {
		_, err := cloud.CloneVolume(test.volName, test.volType, test.srcVolId, test.zone)
